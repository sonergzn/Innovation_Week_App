...
@section('content')
    ...
    <div class=" w-4/5 m-auto text-center">
        <div class="py-16 border-b border-gray-200">
            <h1 class="font-bold font-sans break-normal text-gray-900 pt-6 pb-2 text-3xl md:text-4xl">Workspace</h1>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        @endif
        <div class="mt-6 mb-4">
            <p class="text-base md:text-sm text-blue-500 font-bold">&lt <a href="{{route('index')}}" class="text-base md:text-sm text-blue-500 font-bold no-underline hover:underline">BACK</a></p>
        </div><br>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-6 md:grid-cols-6 lg:grid-cols-6 xl:grid-cols-6 gap-4 mt-8 mb-8">
                   ...
                   <a href="{{ URL::temporarySignedRoute('posts.show', now()->addMinutes(30), ['id' => $post->id]) }}" class="text-blue-500 inline-flex items-center mt-4">Keep Reading
                    ...
                    <div class="col-span-2 sm:col-span-1 xl:col-span-1">
                        @if (isset(Auth::user()->id) && $post->isAuthor(Auth::user()))
                            <span class="float-right">
                                    <a href="{{ URL::temporarySignedRoute('posts.editors', now()->addMinutes(30), ['id' => $post->id]) }}"
                                       class="text-blue-700 hover:text-blue-900 ml-5 pb-0.5 border-b-2">Manage Editors
                                    </a>
                                </span>
                        @endif
                    </div>
